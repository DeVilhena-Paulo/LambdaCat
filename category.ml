...
(**
   ...
   every object ['a] there is exactly one morphism [it] from ['a] to
   [t].
...
= struct
  ...
  let exl () () =
    (fun (x, y) -> x) (* or simply [fst] *)

  let exr () () =
    (fun (x, y) -> y) (* or simply [snd] *)
...
module CartesianCatDerivedOperations (C : CartesianCat) = struct
  ...
  let fork oka okc okd (f : ('a, 'c) C.k) (g : ('a, 'd) C.k) : ('a, 'c * 'd) C.k =
    C.(compose oka (ok_pair oka oka) (ok_pair okc okd)
         (pair oka oka okc okd f g) (dup oka))

  let unfork oka okc okd (h : ('a, 'c * 'd) C.k) : (('a, 'c) C.k) * (('a, 'd) C.k) =
...
module CoCartesianCatDerivedOperations (C : CoCartesianCat) = struct
  ...
  let join oka okc okd ((f : ('c, 'a) C.k), (g : ('d, 'a) C.k)) : (('c * 'd), 'a) C.k =
    C.(compose (ok_pair okc okd) (ok_pair oka oka) oka
           (jam oka) (pair okc okd oka oka f g))

  let unjoin oka okc okd (h : (('c * 'd), 'a) C.k) : (('c, 'a) C.k) * (('d, 'a) C.k) =
...
= struct
  ...
  let ok_pair (type a b) (oka : a ok) (okb : b ok) : (a * b) ok =
    (module (AdditivePair (val oka : Additive with type t = a)
                          (val okb : Additive with type t = b)))
  ...
  let ok_arrow (type a b) (oka : a ok) (okb : b ok) : (a -> b) ok =
    (module AdditiveLambda (struct type t = a end) (val okb : Additive with type t = b))
...
= struct
  ...
  let ok_pair (type a b) (oka : a ok) (okb : b ok) : (a * b) ok =
    (AdditiveFunctionsCat.ok_pair (fst oka) (fst okb), C.ok_pair (snd oka) (snd okb))

  let id oka =
    linearD (LambdaCat.id ()) (C.id (snd oka))
...
= struct
  ...
  let addC =
    linearD (LambdaCat.uncurry () () () C.Num.add) C.addC
...
= struct
  ...
  let unit_arrow (type a) (oka : a ok) (a : a) : (unit, a) k =
    cont ok_unit oka (C.unit_arrow oka a)
