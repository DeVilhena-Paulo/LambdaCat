...
type program_with_locations =
  (binding Position.located * term' Position.located) list
...
let make_lambda_abstraction_with_locations (bs : (binding Position.located) list) t =
  let aux b acc =
    Position.(with_pos (join (position b) (position t)) (Lam (value b, acc))) in
  List.fold_right aux bs t
 (*
  let rec aux = function
    | [] -> t
    | b :: bs ->
       Position.(with_pos (join (position b) (position t)) (Lam (value b, aux bs))) in
  aux bs *)
